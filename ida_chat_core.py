...
# ida-mcp exposes the IDA runtime as a stdio MCP server. The Agent SDK runs
# this server, executes the tools, and feeds results back to the model.
MCP_SERVER_NAME = "ida"
...
def _mcp_server_config(db_path: str) -> dict:
    """Build the stdio launch config for the IDA MCP server.

    The server is launched via the `ida-mcp` console script for the current
    interpreter. This works inside IDA, where `sys.executable` is the IDA binary
    rather than a Python interpreter, because the console script carries the
    correct interpreter in its shebang / `.exe` wrapper. The target database is
    opened and activated before the server begins handling tool calls.
    """
    command = _find_console_script("ida-mcp")
    if not command:
        raise RuntimeError(
            "Could not find the 'ida-mcp' console script for this "
            "Python. Ensure the ida-mcp package is installed."
        )

    return {
        "type": "stdio",
        "command": command,
        "args": ["stdio", "--database", db_path, "--agent=ida-chat"],
    }
...
def _load_system_prompt(db_path: str | None = None) -> str:
    ...
    # activates it on startup, so the agent should not open another database.
    if db_path:
        prompt += (
...
async def _inject_session_meta(input_data, tool_use_id, context):
    """Link IDA MCP tool calls to this agent's semantic trace.

    This mirrors the ida-mcp Claude plugin's PreToolUse hook, but runs
    ...
    IDA MCP dashboard can correlate its session with this conversation.
    """
    if input_data.get('hook_event_name') != 'PreToolUse':
        return {}
...
class IDAChatCore:
    ...
    def __init__(
        ...
        # Correlates this chat with the IDA MCP semantic session.
        self._mcp_id = uuid.uuid4().hex[:12]
    ...
    async def connect(self) -> None:
        ...
        # Tag the MCP session so its telemetry records this chat. The subprocess
        # inherits this env (it is spawned by the SDK's CLI, which inherits ours).
        os.environ["IDA_MCP_ID"] = self._mcp_id
